...
#    if args.optimizer == 'adam':
#        lr = lr * 0.1
#        optimizer = torch.optim.Adam(fpn.parameters(), lr=lr, momentum=0, weight_decay=weight_decay)
#    elif args.optimizer == 'sgd':
#        optimizer = torch.optim.SGD(fpn.parameters(), lr=lr, momentum=0, weight_decay=weight_decay)
...
#            if iteration % 10 == 0:
#                print("Epoch[{}]({}/{}):Loss:{:.4f}, learning rate={}".format(epoch, iteration, len(train_loader), loss.data, lr))

#        print("Epoch {} Complete:Avg.Loss:{:.4f}, learning rate={}".format(epoch, epoch_loss / len(train_loader), lr))

#        def save_checkpoint(epoch):
#            model_out_path = output_dir + "/model_epoch_{}.pth".format(epoch)
#            torch.save(fpn.state_dict(), model_out_path)
